...
namespace UnityEngine.Rendering.Universal
{
    ...
    public sealed class UniversalRenderer : ScriptableRenderer
    {
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            {
                RTHandle cameraTargetHandle = cameraData.xr.CameraTarget;
            ...
            if (this.actualRenderingMode == RenderingMode.Deferred || requiresDepthPrepass || requiresDepthCopyPass)
            {
                ...
                if (RTHandleNeedsRealloc(m_DepthTexture, depthDescriptor, false))
                {
                    m_DepthTexture?.Release();
            ...
            if (requiresDepthPrepass && renderPassInputs.requiresNormalsTexture)
            {
                ...
                {
                    if (RTHandleNeedsRealloc(m_NormalsTexture, normalDescriptor, false))
                    {
                        m_NormalsTexture?.Release();
                ...
                {
                    if (RTHandleNeedsRealloc(m_DeferredLights.GbufferAttachments[(int)m_DeferredLights.GBufferNormalSmoothnessIndex], normalDescriptor, false))
                    {
            ...
            if (generateColorGradingLUT)
            {
                ...
                if (RTHandleNeedsRealloc(m_PostProcessPasses.m_ColorGradingLut, desc, false))
                {
            ...
            if (copyColorPass)
            {
                ...
                if (RTHandleNeedsRealloc(m_OpaqueColor, descriptor, false))
                {
                    m_OpaqueColor?.Release();
            ...
            if (applyPostProcessing)
            {
                ...
                if (RTHandleNeedsRealloc(afterPostProcessColor, desc, false))
                {
            ...
            if (lastCameraInTheStack)
            {
                ...
                bool cameraTargetResolved =
                    ...
                    m_ActiveCameraColorAttachment.nameID == cameraData.xr.CameraTarget.nameID;
                ...
                if (cameraData.xr.enabled)
                {
                    bool depthTargetResolved =
                        ...
                        m_ActiveCameraDepthAttachment == cameraData.xr.CameraTarget.nameID;

                    if (!depthTargetResolved && cameraData.xr.copyDepth)
                    {
                        m_XRCopyDepthPass.Setup(m_ActiveCameraDepthAttachment, cameraData.xr.CameraTarget);
